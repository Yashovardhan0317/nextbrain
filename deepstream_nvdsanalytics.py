...
MUXER_OUTPUT_WIDTH=2560
MUXER_OUTPUT_HEIGHT=1440
    ...
    while l_frame:
        ...
        while l_obj:
            try: 
                ...
                break
               # Only count if the tracking ID is not already in the set
            if obj_meta.class_id == PGIE_CLASS_ID_PERSON and obj_meta.object_id not in unique_tracking_ids:
                obj_counter[obj_meta.class_id] += 1
                unique_tracking_ids.add(obj_meta.object_id)
        ...
        py_nvosd_text_params.display_text = "Exit = {} Entry ={}".format(user_meta_data.objLCCumCnt['Exit'],user_meta_data.objLCCumCnt['Entry'])

        # Now set the offsets where the string should appear
        py_nvosd_text_params.x_offset = 20
        py_nvosd_text_params.y_offset = 1000
        ...
        py_nvosd_text_params.text_bg_clr.set(0.0, 0.0, 1.0, 1.0)
        # Using pyds.get_string() to get display_text as string
        print(pyds.get_string(py_nvosd_text_params.display_text))
        pyds.nvds_add_display_meta_to_frame(frame_meta, display_meta)
        
        print("Frame Number=", frame_number, "stream id=", frame_meta.pad_index, "Number of Objects=",num_rects,"Person_count=",obj_counter[PGIE_CLASS_ID_PERSON])
